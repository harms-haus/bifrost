import { describe, it, expect, vi, beforeEach } from "vitest";
import { render, screen, waitFor, fireEvent } from "@testing-library/react";
import { ReactNode } from "react";

// Mock the auth hooks
const mockAuthState = {
  session: null as {
    username: string;
    account_id: string;
    is_sysadmin: boolean;
  } | null,
  isAuthenticated: false,
  isLoading: false,
};

// Mock API client
const mockApiState = {
  getRealms: vi.fn(),
  createRealm: vi.fn(),
};

vi.mock("@/lib/auth", () => ({
  useAuth: () => mockAuthState,
}));

vi.mock("@/lib/api", () => ({
  ApiClient: class MockApiClient {
    getRealms = mockApiState.getRealms;
    createRealm = mockApiState.createRealm;
  },
}));

// Router wrapper for testing
const RouterWrapper = ({ children }: { children: ReactNode }) => (
  <div>{children}</div>
);

// Mock window.location
let mockHref = "";
Object.defineProperty(window, "location", {
  value: {
    ...window.location,
    get href() { return mockHref; },
    set href(value: string) { mockHref = value; },
  },
  writable: true,
});

// Import Page after mocks are set up
const { Page } = await import("../+Page");

// Mock data
const mockRealms = [
  {
    realm_id: "realm-1",
    name: "Production",
    status: "active" as const,
    created_at: "2024-01-01T00:00:00Z",
  },
  {
    realm_id: "realm-2",
    name: "Development",
    status: "suspended" as const,
    created_at: "2024-02-01T00:00:00Z",
  },
];

describe("Realms List Page", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockAuthState.session = null;
    mockAuthState.isAuthenticated = false;
    mockAuthState.isLoading = false;
    mockApiState.getRealms.mockReset();
    mockApiState.createRealm.mockReset();
    mockHref = "";
  });

  describe("when not authenticated", () => {
    it("shows login prompt", () => {
      render(<Page />, { wrapper: RouterWrapper });
      expect(screen.getByText(/log in/i)).toBeDefined();
    });
  });

  describe("when authenticated but not sysadmin", () => {
    beforeEach(() => {
      mockAuthState.isAuthenticated = true;
      mockAuthState.session = {
        username: "regularuser",
        account_id: "acct-regular",
        is_sysadmin: false,
      };
    });

    it("shows access denied message", () => {
      render(<Page />, { wrapper: RouterWrapper });
      expect(screen.getByText(/access denied/i)).toBeDefined();
    });
  });

  describe("when authenticated as sysadmin", () => {
    beforeEach(() => {
      mockAuthState.isAuthenticated = true;
      mockAuthState.session = {
        username: "sysadmin",
        account_id: "acct-sysadmin",
        is_sysadmin: true,
      };
      mockApiState.getRealms.mockResolvedValue(mockRealms);
    });

    it("fetches realms on mount", async () => {
      render(<Page />, { wrapper: RouterWrapper });

      await waitFor(() => {
        expect(mockApiState.getRealms).toHaveBeenCalled();
      });
    });

    it("shows realms list", async () => {
      render(<Page />, { wrapper: RouterWrapper });

      await waitFor(() => {
        expect(screen.getByText("Production")).toBeDefined();
        expect(screen.getByText("Development")).toBeDefined();
      });
    });

    it("shows loading state while fetching", () => {
      mockApiState.getRealms.mockImplementation(() => new Promise(() => {}));
      render(<Page />, { wrapper: RouterWrapper });
      expect(screen.getByText(/loading/i)).toBeDefined();
    });

    it("shows create realm button", async () => {
      render(<Page />, { wrapper: RouterWrapper });

      await waitFor(() => {
        expect(screen.getByText(/create realm/i)).toBeDefined();
      });
    });

    it("navigates to realm detail when realm is clicked", async () => {
      render(<Page />, { wrapper: RouterWrapper });

      await waitFor(() => {
        expect(screen.getByText("Production")).toBeDefined();
      });

      fireEvent.click(screen.getByText("Production"));
      expect(mockHref).toBe("/ui/admin/realms/realm-1");
    });
  });
});
