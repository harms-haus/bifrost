...
class ApiClient {
  ...
  async getSession() {
    return this.request("/ui/session", {
      method: "GET"
    });
  }
  ...
  async createRune(request) {
    return this.request("/create-rune", {
      method: "POST",
      body: JSON.stringify(request)
    });
  }
  ...
  async createRealm(request) {
    const response = await this.request("/create-realm", {
      method: "POST",
      body: JSON.stringify(request)
    });
...
function AuthProvider({ children }) {
  ...
  const value = {
    ...
    isSysadmin: (session == null ? void 0 : session.is_sysadmin) ?? false,
    login,
    logout,
    loading
  };
...
function RealmProvider({ children }) {
  ...
  useEffect(() => {
    const applyRealmSelection = (realms) => {
    ...
    const init = async () => {
      ...
      try {
        ...
        const filteredRealms = realms.map((realm) => realm.id).filter((id) => id !== "_admin");
